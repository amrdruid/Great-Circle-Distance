...
describe Services::GreatCircleDistance do
  before(:all) do
    # This array contains 5 customers within the range of the company
  ...
  describe 'constants' do
    it 'validates OFFICE_LATITUDE' do
      expect(Services::GreatCircleDistance::OFFICE_LATITUDE).to eq(53.339428)
    end
    it 'validates OFFICE_LONGITUDE' do
      expect(Services::GreatCircleDistance::OFFICE_LONGITUDE).to eq(-6.257664)
    end
  ...
  describe 'logic' do
    let(:obj) { Services::GreatCircleDistance.new(@customers) }
    ...
    describe '.generate_valid_customers_output_file' do
      TEMP_FILE_PATH = 'spec/samples/data/sorted_customer_output_temp.txt'

      after(:all) do
        File.delete(TEMP_FILE_PATH) if File.exist? TEMP_FILE_PATH
      end

      it 'generates an output file of valid customers sorted by user_id' do
        obj.generate_customers_to_invite
        obj.generate_valid_customers_output_file(TEMP_FILE_PATH)
        expect(IO.read('spec/samples/data/sorted_customers_output.txt')).to eq(IO.read(TEMP_FILE_PATH))
      end
    end
